...
std::unordered_map<uint64, uint64> count[N];
  ...
  for (int cas = 1; cas <= T; ++cas) {
    ...
    if (p[0] == -1) {
      for (int i = 0; i < n; ++i) if (candidate >> i & 1) {
        count[0].emplace(uint64(1) << i, uint64(1));
      }
    } else {
      count[0].emplace(uint64(1) << p[0], uint64(1));
    }
    ...
    for (int i = 1; i < n; ++i) {
      if (p[i] == -1) {
        ...
        for (auto &&e: count[i - 1]) {
          ...
          ns &= mask;
          if ((ns | s) == s) {
            count[i][s | (uint64(1) << p[i])] += e.second;
          }
        }
      }
    }
