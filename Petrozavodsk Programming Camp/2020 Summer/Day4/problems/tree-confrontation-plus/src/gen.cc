...
gen::tree::edge_t combine(int n, int part) {
  ...
  for (auto &&n: ns) {
    ...
    if (type == 0) {
      t_edges = gen::tree::rand_with_dsu(n, std::min(n, std::min(n, rnd.next(1, 5))));
    } else if (type == 1) {
      ...
      t_edges = gen::tree::rand_caterpillar(n, std::max(3, rnd.next(n / 4, n / 3)));
    } else if (type == 5) {
      t_edges = gen::tree::rand_skew_tree(n, std::max(1, rnd.next(n / 4, n / 2)));
    } else if (type == 6) {
...
int main(int argc, char* argv[]) {
  ...
  for (auto &&n: ns) {
    ...
    if (type == 0) {
      ...
      edges = gen::tree::rand_caterpillar(n, std::max(3, rnd.next(n / 4, n / 3)));
    } else if (type == 5) {
    ...
    n = edges.size() + 1;
    int root = rnd.next(n);
    if (type == 2 || type == 3) root = 0;
    auto parent = gen::tree::convert_rooted(n, edges, root, true);
    printf("%d\n", n);
    for (int i = 1; i < n; ++i) {
      if (i > 1) putchar(' ');
      printf("%d", parent[i] + 1);
